...
const PersonForm = ({ newName, setNewName, newNumber, setNewNumber,
    persons, setPersons, setFilteredPersons, setNotification, setError }) => {
    const addPerson = (event) => {
        event.preventDefault()
        const found = persons.find(e => e.name === newName)
        if (found) {
            const dialog = window.confirm(`${newName} is already added to phonebook, replace the old nmber with a new one?`)
            if (dialog)
                phonebookService
                    .update(found.id, { ...found, number: newNumber })
                    .then(response => {
                        const updated = persons.map(person => person.id === found.id ? response : person)
                        setPersons(updated)
